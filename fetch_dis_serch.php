					...
					<tbody>
						<?php
							...
							if(isset($_GET["submit"]))
							{
								...
								$taluka=$_GET["t_name"];
								$village=$_GET["v_name"];
								$qry="select * from scientist where dis_name like '%".$district."%' and t_name like '%".$taluka."%' and v_name like '%".$village."%'";
								//echo $qry;
							}
							else
							{
									$qry="select * from scientist";
							}
							$count=1;
							$rc=mysql_query($qry);
							while($data=mysql_fetch_assoc($rc))
